...
export default function ApLedgerPage() {
  ...
  }, [eRow.amount, eTaxRateKey]);

  async function load() {
    const data = await api<Record<string, unknown>[]>(`/api/ap-ledger?month=${month}`);
    setRows(data);
  }
  ...
  function onMonthChange(next: string) {
    setMonth(next);
    router.replace(`/accounting/ap?month=${encodeURIComponent(next)}`);
  }

  async function saveApRow(rowId: string) {
    await runSave(
      () =>
        api(`/api/ap-ledger/${rowId}`, {
          method: 'PATCH',
          body: JSON.stringify({
            supplierId: eRow.supplierId,
            closingDay: eRow.closingDay ? Number(eRow.closingDay) : null,
            amount: eRow.amount,
            taxAmount: eRow.taxAmount,
            totalAmount: eRow.totalAmount || eRow.amount + eRow.taxAmount,
          }),
        }),
      async () => {
        setEditingId(null);
  ...
  const newHref = `/accounting/ap/new?month=${encodeURIComponent(month)}`;

  return (
    <>
      <PageTitle title="買掛金管理" description="一覧で月を選び、行から編集します。新規は入力画面で登録します。" />
                ...
                return (
                  <tr key={id} className="border-b border-slate-100 bg-aqua-50/30">
                    <td className="px-4 py-3 align-top">
                      <select
                        value={eRow.supplierId}
                        onChange={(e) => setERow((p) => ({ ...p, supplierId: e.target.value }))}
                    ...
                    <td className="px-4 py-3 align-top text-right">
                      ...
                      <CurrencyInput
                        value={eRow.amount}
                        onChange={(n) => setERow((p) => ({ ...p, amount: n }))}
                    ...
                    <div className="flex flex-wrap justify-end gap-2">
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (amount > 0 && taxAmt === 0) inferred = '0';
                          else if (
                            amount > 0 &&
                            taxAmt === Math.floor(amount * 0.08) &&
                            taxAmt !== Math.floor(amount * 0.1)
                          )
                            inferred = '8';
                          else if (amount > 0 && taxAmt === Math.floor(amount * 0.1)) inferred = '10';
                      ...
                      {r.pdf_data_url ? (
                        <a className="rounded border px-3 py-1.5 text-xs" href={`/api/ap-ledger/${id}/pdf`} target="_blank" rel="noreferrer">
                          PDF
                        </a>
                      ) : null}
                      <button
                        ...
                        onClick={async () => {
                          ...
                          const ok = await runSave(() => apiDelete(`/api/ap-ledger/${id}`), load);
